...
export class MemoryRepository {
  ...
  async change(key, operation) {
    for (let attempt = 0; attempt < 8; attempt++) {
      ...
      for (const row of archive.bookings) this.archivedBookings.set(row.id, structuredClone(row));
  ...
  async listArchivedBookings({ owner, from, to } = {}) {
    return [...this.archivedBookings.values()].filter(r => (!owner || r.owner === owner) && (!from || r.date >= from) && (!to || r.date <= to)).map(r => structuredClone(r));
  }
}
...
export class MemoryDocuments {
  ...
  async listAll() { return [...this.objects.keys()]; }
