...
import { executeBusiness, publicBooking, publicChild, availabilityList, closureOf, capacityOf, audit, event, need, Fault, text, RULES, STATUSES, TERMINAL, HOLD } from './rules.mjs';
...
export const COMMANDS = {
  ...
  staff: ['registration', 'confirm', 'return-documents', 'reject', 'accept-cancel', 'check-in', 'complete', 'message', 'assign', 'offer', 'expire-offer', 'resolve-incident', 'close-day', 'reopen-day', 'capacity'],
};
...
export class FacilityService {
  ...
  // First administrator. Only possible while the facility has no staff at all.
  async bootstrapAdmin(subject, profile = {}) {
    need(typeof subject === 'string' && subject.length > 0, '本人確認が必要です。', 401);
    return this.change(s => {
      if (!s.facilityId) { s.facilityId = this.facilityId; s.memberships = []; s.documentVersions = []; }
      this.checkFacility(s);
      need(!s.memberships.some(m => m.role === 'staff'), '初期管理者は登録済みです。', 409);
      s.memberships.push({ id: uuid(), subject, role: 'staff', permission: 'admin', active: true, version: 1, name: profile.name || '管理者', email: profile.email || '' });
      audit(s, subject, '施設を初期設定', this.facilityId);
      return { ok: true };
    });
  }
  ...
  async read(subject, role) {
    ...
    const result = { clock: s.clock, children, bookings, rules: { arrivalTimes: RULES.arrivalTimes, endTimes: RULES.endTimes, rooms: RULES.rooms, groups: RULES.groups, dailyCapacity: RULES.dailyCapacity, roomCapacity: RULES.roomCapacity, feeShort: RULES.feeShort, feeLong: RULES.feeLong } };
    ...
    return snapshot({ ...result, permission: actor.permission, me: actor.id, tasks: this.tasks(s), audit: s.audit.slice(-100), closed: s.closed, days,
      households: s.accounts.map(a => ({ id: a.id, email: a.email || '' })),
      staff: s.memberships.filter(m => m.role === 'staff').map(({ id, permission, active, version, name, email }) => ({ id, permission, active, version, name: name || '職員', email: email || '' })), dayCapacity: s.dayCapacity });
  }
  tasks(s) {
  ...
  async command(subject, role, path, body) {
    ...
    return this.change(async s => {
      ...
      if (['close-day', 'reopen-day', 'capacity'].includes(path)) {
        ...
        const changed = [];
        if (path === 'capacity') {
          need(Number.isInteger(body.capacity) && body.capacity >= 0 && body.capacity <= RULES.dailyCapacity, `受入上限は0〜${RULES.dailyCapacity}名です。`, 400);
  ...
  async submitDocuments(subject, { id, version, files }) {
    ...
    need(!booking.medication || kinds.includes('medicine'), '服薬ありの場合、与薬依頼書も必要です。', 400);
