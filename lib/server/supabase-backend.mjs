...
export class SupabaseRepository {
  ...
  async listArchivedBookings({ key, owner, from, to } = {}) {
    let query = this.db.from('supportyou_booking_archive').select('id,owner,date,status,data').eq('facility_key', key).order('date', { ascending: false }).limit(2000);
...
export function createSupabaseBackend(env = process.env) {
  ...
  const service = facilityId => { if (!services.has(facilityId)) services.set(facilityId, new FacilityService(repo, { facilityId, documentStore: documents })); return services.get(facilityId); };
  ...
  const identities = {
    async get(subject) { return check(await db.from('supportyou_identities').select('subject,email,name,kind,blocked,org_role').eq('subject', subject).maybeSingle()); },
    async insert(row) {
    ...
    async setOrgRole(subject, role) { check(await db.from('supportyou_identities').update({ org_role: role }).eq('subject', subject).eq('kind', 'staff')); },
    async owners() { return check(await db.from('supportyou_identities').select('subject,email').eq('org_role', 'owner').eq('blocked', false)); },
  };

  const facilities = {
    async list() { return check(await db.from('supportyou_facilities').select('id,name,municipality,active,created_at').order('created_at')); },
    async get(id) { return check(await db.from('supportyou_facilities').select('id,name,municipality,active').eq('id', id).maybeSingle()); },
    async create(row) { const { error } = await db.from('supportyou_facilities').insert(row); if (error) fail(error.code === '23505' ? 'この施設IDは使用済みです。' : '施設を登録できません。', error.code === '23505' ? 409 : 503); },
    async update(id, row) { check(await db.from('supportyou_facilities').update(row).eq('id', id)); },
  };

  const invites = {
    async create(row) {
      let stale = db.from('supportyou_staff_invites').update({ revoked_at: new Date().toISOString() }).eq('email', row.email).is('used_at', null).is('revoked_at', null);
      stale = row.facility_id ? stale.eq('facility_id', row.facility_id) : stale.is('facility_id', null);
      check(await stale);
      return check(await db.from('supportyou_staff_invites').insert(row).select('id,email,name,permission,facility_id,expires_at').single());
    },
    async list(facilityId) {
      let query = db.from('supportyou_staff_invites').select('id,email,name,permission,facility_id,expires_at,created_at,used_at,revoked_at').is('used_at', null).is('revoked_at', null).order('created_at', { ascending: false }).limit(50);
      query = facilityId ? query.eq('facility_id', facilityId) : query.is('facility_id', null);
      return check(await query);
    },
    async revoke(id, facilityId) {
      let query = db.from('supportyou_staff_invites').update({ revoked_at: new Date().toISOString() }).eq('id', id).is('used_at', null).is('revoked_at', null);
      query = facilityId ? query.eq('facility_id', facilityId) : query.is('facility_id', null);
      return check(await query.select('id')).length === 1;
    },
  ...
  const config = {
    ...
    async setBootstrap(subject) {
      ...
      if (error) fail('本部の管理者は登録済みです。', 409);
    },
  };
  ...
  return {
    kind: 'supabase', service, facilities, repo, documents, auth, identities, invites, config, orphans,
    ...
    async listNotifications(facilityId) { return check(await db.from('supportyou_notifications').select('id,kind,recipient_role,status,error,created_at').eq('facility_id', facilityId).order('created_at', { ascending: false }).limit(30)); },
  };
}
