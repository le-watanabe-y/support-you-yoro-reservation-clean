...
// Messages never contain names, symptoms or other medical details: only the date,
// the new state and a link back to the signed-in site.
import { FACILITY } from '../facility-info.mjs';
...
export function parentMessage({ date, label, origin }) {
  return {
    subject: `【${FACILITY.shortName}】予約状況が更新されました`,
    text: `${date ? `${dateLabel(date)}のご予約（お申込み）` : 'ご登録内容'}について、状況が「${label}」に更新されました。\n\n詳細と施設からの連絡は、ログインしてご確認ください。\n${origin}/line?view=history${footer(origin)}`,
  };
}
export function facilityMessage({ date, label, origin }) {
  return {
    subject: `【${FACILITY.shortName}】${label}（${date}）`,
    text: `${dateLabel(date)}の予約について「${label}」がありました。\n職員画面で確認してください。\n${origin}/staff${footer(origin)}`,
  };
}
export function inviteMessage({ name, code, origin, expiresAt }) {
  return {
    subject: `【${FACILITY.shortName}】職員用アカウントの招待`,
    text: `${name} 様\n\n${FACILITY.name}の職員画面への招待です。\n次のページで「招待を受けた方」を選び、このメールアドレス・招待コード・新しいパスワードを入力してください。\n\n${origin}/staff\n招待コード：${code}\n有効期限：${new Date(expiresAt).toLocaleString('ja-JP', { timeZone: 'Asia/Tokyo' })}\n\n心当たりがない場合は、このメールを破棄してください。${footer(origin)}`,
  };
}
