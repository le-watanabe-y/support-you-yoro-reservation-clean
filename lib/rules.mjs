...
import { at, jstDate, jstTime, addDays, isCalendarDate, holidayName, weekday } from './calendar.mjs';
import { DEFAULT_SETTINGS, closingTime, feeFor } from './settings.mjs';

export const settingsOf = s => s.settings || DEFAULT_SETTINGS;
export const roomsOf = s => settingsOf(s).rooms.map(r => r.name);
...
export const newState = () => ({ settings: null, clock: new Date().toISOString(), accounts: [], children: [], bookings: [], audit: [], receipts: [], closed: [], dayCapacity: {}, incidents: [] });
...
export function profile(body, today, settings = DEFAULT_SETTINGS) {
  const birth = text(body.birth, '生年月日', 10);
  need(isCalendarDate(birth) && birth <= today && birth > addDays(today, -366 * (settings.ageMaxYears + 1)), `生年月日を確認してください。対象は${settings.ageMaxYears}歳までです。`);
  return {
...
export const capacityOf = (s, date) => s.dayCapacity?.[date] ?? settingsOf(s).dailyCapacity;
const moment = (date, m) => at(addDays(date, -m.daysBefore), m.time);
export function closureOf(s, date) {
  const set = settingsOf(s);
  if (s.closed.includes(date)) return '施設の休園日';
  if (!set.openWeekdays.includes(weekday(date))) return '休園曜日';
  const md = date.slice(5), y = set.yearEndClosure;
  if (y.enabled && (y.from <= y.to ? md >= y.from && md <= y.to : md >= y.from || md <= y.to)) return '年末年始';
  if (set.closeOnHolidays) { const name = holidayName(date); if (name) return name; }
  return '';
}
// Age in whole months on a date.
export const ageMonths = (birth, date) => { const [by, bm, bd] = birth.split('-').map(Number), [y, m, d] = date.split('-').map(Number); return (y - by) * 12 + (m - bm) - (d < bd ? 1 : 0); };

export function availability(s, date) {
  const set = settingsOf(s), now = Date.parse(s.clock);
  if (closureOf(s, date)) return '休園';
  if (date < jstDate(s.clock)) return '受付終了';
  if (now < moment(date, set.bookingOpens)) return '受付開始前';
  if (now > moment(date, set.bookingDeadline) || now >= at(date, closingTime(set)) || !set.arrivalTimes.some(start => at(date, start) >= now)) return '受付終了';
  if (s.bookings.filter(b => b.date === date && HOLD.includes(b.status)).length >= capacityOf(s, date)) return set.waitlist ? 'キャンセル待ち' : '満員';
  return '予約可';
}
export const availabilityList = s => Array.from({ length: settingsOf(s).listDays }, (_, i) => {
...
export async function executeBusiness(s, user, path, body) {
  const role = user.role, set = settingsOf(s);
  ...
  if (role === 'parent') {
    if (path === 'register-child') {
      ...
      need(s.children.filter(c => c.owner === user.id).length < 6, '1家庭6名まで登録できます。');
      const p = profile(body, today, set);
    ...
    if (path === 'edit-child') {
      ...
      const updated = profile(body, today, set);
    ...
    if (path === 'reserve') {
      ...
      need(set.arrivalTimes.includes(body.start) && set.endTimes.includes(body.end) && body.end > body.start, '利用時刻を確認してください。');
      const months = ageMonths(c.birth, body.date);
      need(months >= set.ageMinMonths && months < (set.ageMaxYears + 1) * 12, `対象年齢（生後${set.ageMinMonths}か月〜${set.ageMaxYears}歳）の範囲外です。施設へご相談ください。`, 409);
      ...
      const b = { id: uuid(), owner: user.id, childId: c.id, date: body.date, start: body.start, end: body.end, symptom, notes, medication: body.medication, physician: false, medicineDoc: false, status: open === 'キャンセル待ち' ? 'waitlisted' : 'pending', version: 1, rulesVersion: set.version, messages: [], events: [{ at: s.clock, label: '申込み受付（未確定）' }], createdAt: s.clock };
    ...
    if (path === 'documents') {
      ...
      need(!b.medication || !set.requireMedicineDoc || body.medicineDoc === true, '服薬ありの場合、与薬依頼書も必要です。');
      b.physician = true; b.medicineDoc = b.medication && body.medicineDoc === true;
    ...
    if (path === 'cancel') {
      ...
      if (Date.parse(s.clock) >= moment(b.date, set.cancelRequestFrom) && HOLD.includes(b.status)) b.status = 'cancel_requested';
    ...
    if (path === 'confirm') {
      ...
      need(b.physician && (!b.medication || !set.requireMedicineDoc || b.medicineDoc), '必要書類が揃っていません。', 409);
      ...
      const room = chosen(body.room, roomsOf(s), '保育室'), group = chosen(body.group, set.groups, '感染区分');
      const roomCapacity = set.rooms.find(r => r.name === room).capacity;
      ...
      need(occupants.length < roomCapacity, '選択した保育室は定員に達しています。', 409);
      need(!set.separateGroups || occupants.every(x => x.group === group), '異なる感染区分を同じ保育室に配置できません。', 409);
    ...
    if (path === 'check-in') {
      need(b.status === 'confirmed' && !closureOf(s, b.date) && b.date === today && jstTime(s.clock) >= addMinutes(b.start, -set.checkInEarlyMinutes) && jstTime(s.clock) < b.end, '休園日以外の利用当日・予約時間内・確定済みの場合に受付できます。', 409);
    ...
    if (path === 'complete') {
      ...
      b.fee = feeFor(set, Date.parse(s.clock) - Date.parse(b.checkedInAt));
      b.status = 'completed';
