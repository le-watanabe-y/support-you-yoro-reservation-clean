...
import { lineEntry } from '@/lib/line-entry.mjs';
...
export function PersistentParentTabs(props:ComponentProps<typeof BaseTabs>){
 ...
 useEffect(()=>{let saved='';if(location.pathname==='/line'||location.pathname==='/line/')saved=lineEntry(location.search).tab;else try{saved=localStorage.getItem(storageKey)||'';}catch{}const timer=setTimeout(()=>{if(allowed.has(saved))setValue(saved);if(location.pathname==='/line'||location.pathname==='/line/')revealById(lineEntry(location.search).target);},0);return()=>clearTimeout(timer);},[]);
