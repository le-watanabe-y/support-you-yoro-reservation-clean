...
import { FacilitySwitcher, Headquarters, JoinPanel, SettingsPanel } from './org-controls';
...
export default function StaffView({ api, facility, setFacility }: { api: Api, facility: string, setFacility: (id: string) => void }) {
  ...
  const profile = s?.settings?.profile ? { id: facility, ...s.settings.profile } : null;
  const open = (id: string) => { setFilter('対応待ち'); setDate('全日'); setFacility(id); };
  return <Frame staff facility={profile} authenticated={s?.authenticated} onLogout={() => api.act('logout', {}, 'ログアウトしました。')}>
    <Feedback api={api} />
    {!s ? (!api.error && <p role="status">読み込んでいます…</p>) : !s.authenticated ? <div className="staff-entry"><AccountForm key={String(s.setupAvailable)} api={api} staff /></div> : !s.settings ? <>
      <div className="page-title"><div><p className="eyebrow">職員用 · {s.user.email}</p><h1>{s.owner ? '本部' : '施設を選んでください'}</h1></div></div>
      {s.facilities.length ? <Panel title="施設"><div className="facility-list">{s.facilities.map((f: any) => <button type="button" key={f.id} className="facility-card" onClick={() => open(f.id)}><strong>{f.name}</strong><span>{f.municipality}{f.active ? '' : '（停止中）'}</span></button>)}</div></Panel> : <Blank title="利用できる施設がありません" description="管理者から受け取った招待コードを入力してください。" />}
      <Headquarters api={api} onOpen={open} /><JoinPanel api={api} onJoined={open} />
    </> : <>
      <div className="page-title"><div><p className="eyebrow">{s.facility?.name} · {s.staff.find((m: any) => m.id === s.me)?.name}{s.owner ? '（本部）' : ''}</p><h1>受付とお預かりの管理</h1></div><div className="title-actions"><FacilitySwitcher api={api} facility={facility} onChange={open} /><Button variant="outline" disabled={api.busy} onClick={api.refresh}>最新情報に更新</Button></div></div>
      <div className="stats">
      ...
      <Tabs key={facility} defaultValue="reservations"><TabsList className="main-tabs"><TabsTrigger value="reservations">予約管理</TabsTrigger><TabsTrigger value="calendar">開園日・出力</TabsTrigger><TabsTrigger value="rules">ハウスルール</TabsTrigger><TabsTrigger value="audit">操作履歴</TabsTrigger><TabsTrigger value="settings">職員・設定</TabsTrigger>{s.owner && <TabsTrigger value="hq">本部</TabsTrigger>}</TabsList>
        ...
        <TabsContent value="calendar"><FacilityCalendar api={api} /><ExportPanel facility={facility} /></TabsContent>
        <TabsContent value="rules"><SettingsPanel api={api} /></TabsContent>
        <TabsContent value="audit"><Panel title="操作履歴（直近100件）"><p className="hint">誰がいつ何を操作したかを記録しています。</p><Table><TableHeader><TableRow><TableHead>日時</TableHead><TableHead>操作者</TableHead><TableHead>操作</TableHead></TableRow></TableHeader><TableBody>{s.audit.slice().reverse().map((a: any) => { const staff = s.staff.find((m: any) => m.id === a.actor || m.subject === a.actor); const household = s.households.find((h: any) => h.id === a.actor); return <TableRow key={a.id}><TableCell>{clockLabel(a.recordedAt || a.at)}</TableCell><TableCell>{staff ? `職員 ${staff.name}` : household ? `保護者 ${household.email}` : a.actor === 'system' ? 'システム' : '保護者・本部'}</TableCell><TableCell>{a.event}</TableCell></TableRow>; })}</TableBody></Table></Panel></TabsContent>
        <TabsContent value="settings"><StaffAdmin api={api} facility={facility} />{s.permission === 'admin' && <LineSetup facility={facility} />}<JoinPanel api={api} onJoined={open} /></TabsContent>
        {s.owner && <TabsContent value="hq"><Headquarters api={api} onOpen={open} /></TabsContent>}
      </Tabs>
    </>}
  </Frame>;
}
