...
export function AccountForm({ api, staff = false }: { api: Api, staff?: boolean }) {
  ...
  const title = { login: staff ? '職員ログイン' : 'ログイン', signup: 'はじめての方：アカウント作成', reset: 'パスワードの再設定', activate: '招待を受けた職員の利用開始', setup: '初期管理者の登録' }[mode];
  ...
  return <Panel title={title} step={mode}>
    <p>{mode === 'login' ? (staff ? '職員用のメールアドレスでログインしてください。' : '登録済みの方はメールアドレスとパスワードでログインしてください。') : mode === 'signup' ? 'メールアドレスとパスワードを設定します。登録後、お子さまの情報を入力します。' : mode === 'reset' ? '登録したメールアドレスに、再設定用のリンクを送ります。' : mode === 'activate' ? '管理者から受け取った招待コードで、職員用アカウントを作成します。' : '施設で最初の管理者を登録します。配備時に設定した初期設定コードが必要です。'}</p>
    <Form key={mode} onSubmit={async e => {
      ...
      const r = await api.act(action, { email: f.email, password: f.password, consent: f.consent === 'yes', code: f.code, name: f.name, setupCode: f.setupCode }, mode === 'reset' ? '送信しました。' : mode === 'signup' ? 'アカウントを作成しました。' : 'ログインしました。');
      if (!r) return;
      ...
      <fieldset disabled={api.busy}>
        ...
        {mode === 'setup' && <Text label="初期設定コード" name="setupCode" minLength={16} maxLength={200} autoComplete="off" autoCapitalize="none" spellCheck={false} />}
    ...
    <div className="login-links">
      ...
      {mode === 'login' && staff && api.state?.setupAvailable && <Button variant="outline" onClick={() => setMode('setup')}>初期管理者を登録</Button>}
...
export function DocumentUpload({ b, api }: { b: any, api: Api }) {
  ...
  return <Form onSubmit={async e => {
    ...
    if (b.medication && !medicine) { setError('服薬ありの場合、与薬依頼書も選んでください。'); return; }
    ...
    {b.medication && <Field label="与薬依頼書"><input className="file-input" type="file" name="medicine" accept="image/*,application/pdf" required /></Field>}
...
export function FacilityCalendar({ api }: { api: Api }) {
  ...
  return <Panel title="開園日と受入上限（14日間）">
    ...
    {!admin ? <p className="hint">休園・上限の変更は管理者が行います。</p> : <>
      ...
      {date && <><p>{dateLabel(date)}：{s.closed.includes(date) ? '休園（施設設定）' : day?.closure ? `休園（${day.closure}）` : '開園'} / 上限 {s.dayCapacity[date] ?? s.rules.dailyCapacity}名</p>
        ...
        <Form onSubmit={e => { e.preventDefault(); const f = values(e.currentTarget); api.act('capacity', { date, capacity: Number(f.capacity) }, '受入上限を保存しました。'); }}><fieldset disabled={api.busy}><Pick name="capacity" label="この日の受入上限" items={Array.from({ length: s.rules.dailyCapacity + 1 }, (_, i) => String(i))} /><Button variant="outline" type="submit">上限を保存</Button></fieldset></Form></>}
    </>}
  </Panel>;
}

export function StaffAdmin({ api }: { api: Api }) {
  ...
  return <>
    ...
    <Panel title="データの保全">
      ...
      <p><a className="line-open-browser" href="/api/staff/backup" download>全データを書き出す（JSON）</a></p>
