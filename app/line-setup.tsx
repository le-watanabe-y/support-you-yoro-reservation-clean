...
export function LineSetup({ facility }: { facility: string }) {
  const [origin, setOrigin] = useState('');
  const [result, setResult] = useState('');
  useEffect(() => { setOrigin(location.origin); }, []);
  const links = [
    { label: '保護者：予約申込み', path: lineEntryPath(facility, 'reserve') },
    { label: '保護者：予約確認', path: lineEntryPath(facility, 'history') },
    { label: '保護者：利用案内', path: lineEntryPath(facility, 'guide') },
    { label: '職員専用', path: lineEntryPath(facility, 'reserve', true) },
  ];
  ...
  return <Panel title="LINE公式アカウントからの入口（この施設）">
    <p>この施設のLINE公式アカウントのリッチメニューに、下の保護者用リンクを設定します。施設ごとにリンクが異なります。職員用リンクは保護者向けメニューに入れないでください。</p>
    ...
    <details><summary>LINEリッチメニューの設定手順</summary>
      ...
      {origin.startsWith('https://') && <Button variant="outline" onClick={() => copy('Messaging API用のメニュー設定（JSON）', JSON.stringify(buildLineMenu(origin, facility), null, 2))}>Messaging API用の設定（JSON）をコピー</Button>}
