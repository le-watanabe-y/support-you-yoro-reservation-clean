...
import { hoursText, feeText, ageText, momentText } from '@/lib/settings.mjs';
...
function ChildForm({ api, child, onDone }: { api: Api, child?: any, onDone: () => void }) {
  ...
  return <Panel id="child-registration" autoReveal title={child ? '利用者情報の変更' : 'お子さま・保護者の利用登録'}>
    ...
    <Form onSubmit={e => { e.preventDefault(); const f = values(e.currentTarget); setSaved(f); setDraft({ ...f, consent: f.consent === 'yes', emergencyConsent: f.emergencyConsent === 'yes' }); }}><fieldset disabled={api.busy}>
      ...
      {api.state.settings?.eligibilityNote && <p className="note"><strong>この施設の利用条件</strong><br />{api.state.settings.eligibilityNote}</p>}
      <Check name="consent" defaultChecked={saved.consent === 'yes'}>対象年齢（{ageText(api.state.settings)}）・利用条件を確認しました。食物アレルギーがある場合は除去食を持参することを確認しました。</Check>
...
function ReservationForm({ date, api, onDone }: { date: string, api: Api, onDone: () => void }) {
  const rules = api.state.settings;
  ...
  return <Panel id="reservation-step" title={`${dateLabel(date)}の申込み`} aside={<Button variant="ghost" onClick={onDone}>閉じる</Button>}><p className="step-label">手順 1 / 2 · 申込み内容を入力</p>
    <Form noValidate onSubmit={e => { e.preventDefault(); const f = values(e.currentTarget), labels: any = { childId: '利用するお子さま', start: '来園予定', end: 'お迎え予定', symptom: '症状', medication: '服薬の有無', agree: '確認欄' }, missing = Object.keys(labels).filter(k => !f[k]?.trim()); if (missing.length) { setFormError(`未入力の項目があります：${missing.map(k => labels[k]).join('、')}`); return; } if (f.start >= f.end) { setFormError('お迎え予定は、来園予定より後の時刻を選んでください。'); return; } setFormError(''); setSaved(f); setDraft({ ...f, medication: f.medication === 'あり', agree: true }); }}><fieldset disabled={api.busy}>
      ...
      <Pick name="medication" defaultValue={saved.medication} label="お預かり中の服薬" items={['なし', 'あり']} />{rules.requireMedicineDoc && <p className="hint">服薬がある場合は、与薬依頼書の提出が必要です。</p>}
...
function Booking({ b, api }: { b: any, api: Api }) {
  ...
  return <article id={`parent-booking-${b.id}`} className="booking"><div className="booking-head"><div><p className="eyebrow">{dateLabel(b.date)} · {b.start}–{b.end}</p><h3>{c?.name}</h3></div><Badge status={b.status}>{b.label}</Badge></div>
    ...
    {b.status === 'cancel_requested' && <p className="note">取消しはまだ完了していません。施設の確認まで枠を保持します。当日の急ぎのご連絡は、施設へお電話ください{api.state.settings?.profile.phone && `（${api.state.settings.profile.phone}）`}。</p>}
    ...
    {active && !['checked_in', 'cancel_requested'].includes(b.status) && <div className="booking-bottom"><Confirm disabled={api.busy} label="キャンセル" description={`${momentText(api.state.settings.cancelRequestFrom, '以降')}の確定済み予約は「取消し申出」となり、施設の確認まで完了しません。それ以外はすぐにキャンセルされます。`} onConfirm={() => api.act('cancel', { id: b.id, version: b.version }, 'キャンセルの処理状況を更新しました。表示をご確認ください。')} /></div>}
  </article>;
}

function Guide({ api }: { api: Api }) {
  const set = api.state?.settings;
  if (!set) return null;
  const p = set.profile;
  return <Panel id="parent-guide" title="ご利用案内">
    <ol className="guide-list"><li><strong>利用登録</strong><p>保護者・お子さまの情報を登録します。施設が初回のご予約と一緒に内容を確認します。</p></li><li><strong>受診・申込み</strong><p>医療機関を受診し、医師連絡票を記入してもらいます。{momentText(set.bookingOpens, 'から')}{momentText(set.bookingDeadline, 'まで')}申込みできます。</p></li><li><strong>必要書類の提出</strong><p>医師連絡票{set.requireMedicineDoc && '（服薬がある場合は与薬依頼書も）'}を撮影して提出します。</p></li><li><strong>施設が確認して確定</strong><p>書類・症状・保育室の状況を確認して確定します。確定するとメールでお知らせします。</p></li><li><strong>当日</strong><p>来園時に職員がお子さまの様子を確認します。お迎え時は本人確認を行います。</p></li></ol>
    <h3>開所日・時間</h3><p>{hoursText(set)}</p><p>来園：{set.arrivalTimes.join('・')} ／ お迎え：{set.endTimes[0]}〜{set.endTimes[set.endTimes.length - 1]}</p>
    <h3>対象</h3><p>{ageText(set)}{p.municipality && `（${p.municipality}）`}</p>{set.eligibilityNote && <p>{set.eligibilityNote}</p>}
    <h3>定員</h3><p>1日{set.dailyCapacity}名まで。{set.waitlist ? '満員の場合はキャンセル待ちとして受け付け、空きが出たときに意思確認のご連絡をします。' : '満員の場合は申込みできません。'}</p>
    <h3>利用料金</h3><p>{feeText(set)}</p>{set.feeNote && <p>{set.feeNote}</p>}
    <h3>キャンセル</h3><p>画面からキャンセルできます。{momentText(set.cancelRequestFrom, '以降')}の確定済み予約は「取消し申出」となり、施設が確認して完了します。</p>
    {set.guideNote && <><h3>施設からのお知らせ</h3><p className="pre-line">{set.guideNote}</p></>}
    <h3>お問い合わせ</h3><p>{p.name}{p.phone && <> 電話 <a href={`tel:${p.phone}`}>{p.phone}</a></>}{p.address && <><br />{p.address}</>}</p>
  </Panel>;
}
...
export default function ParentView({ api, facilityId }: { api: Api, facilityId: string }) {
  ...
  useEffect(() => { try { localStorage.setItem('supportyou-last-facility', facilityId); } catch { } }, [facilityId]);
  useEffect(() => { if (!login || restored.current === login) return; restored.current = login; if (/\/line\/?$/.test(location.pathname)) { setDate(''); return; } let saved = { date: '', scrollY: 0 }; try { saved = loadParentUi(login, localStorage); } catch { } setDate(s.availability?.some((a: any) => a.date === saved.date) ? saved.date : ''); requestAnimationFrame(() => requestAnimationFrame(() => scrollTo(0, saved.scrollY))); }, [login, s?.availability]);
  ...
  const facility = s?.settings ? { id: facilityId, ...s.settings.profile } : s?.facility ? { id: facilityId, name: s.facility.name } : { id: facilityId };
  return <Frame facility={facility} authenticated={s?.authenticated} onLogout={() => { setDate(''); setAdding(false); setEdit(null); api.act('logout', {}, 'ログアウトしました。'); }}><Feedback api={api} />
    ...
    {!s ? (!api.error && <p role="status">読み込んでいます…</p>) : !s.authenticated ? <div className="entry"><div className="entry-intro"><p className="eyebrow">{s.facility?.name}</p><h1>病児保育の<br />ご予約</h1><ol className="steps"><li className="current">アカウント作成・ログイン</li><li>この施設でのお子さまの利用登録</li><li>受診・申込み・書類の提出</li><li>施設の確認後に予約確定</li></ol><p>ひとつのアカウントで、複数の施設をご利用いただけます。</p></div><AccountForm api={api} /></div> : <>
